...
import { ROUTE_TXN_TYPE } from "./_ssr/common.schema-rOPsTdW8.mjs";
...
import { amountShortener, cn } from "./_ssr/utils-lKLyXhB7.mjs";
import { ArrowRight } from "./_libs/lucide-react.mjs";
import { buttonVariants } from "./_ssr/button-Bhg_Lprh.mjs";
import { useSuspenseQuery } from "./_libs/tanstack__react-query.mjs";
import { Card, CardAction, CardContent, CardDescription, CardFooter, CardHeader, CardTitle, Separator$1, Skeleton, committeeMemberOptions, currDBUserQueryOptions } from "./_ssr/separator-B8iuesUR.mjs";
import { Image } from "./_libs/unpic__react.mjs";
import { committeeBalancesOptions, donationStatsOptions } from "./_ssr/card-loader-B0N-W8_R.mjs";
import { Background } from "./_ssr/background-Bade6QlY.mjs";
import { Badge, SuspenseErrorBoundary } from "./_ssr/suspense-error-boundary-BpLC6vzM.mjs";
import { Amount, AnimatedList, AnimatedListItem, SelectYear } from "./_ssr/animated-list-CYIuENrB.mjs";
import { TxnButton } from "./_ssr/txn-button-CGYoIZAS.mjs";
import { CardStatsLoader } from "./_ssr/card-stats-loader-CQYh2HDu.mjs";
import { Route } from "./_committee._year-B_RcniLE.mjs";
import { Bar, BarChart, CartesianGrid, LabelList, Rectangle, ResponsiveContainer, Tooltip, XAxis } from "./_libs/recharts+[...].mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/_committee._year-Cb9Vu-7_.js
...
function CommitteeCardDescription() {
	...
	if (member?.isActive) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SelectYear, {
		year,
		handleSelect
	});
...
function OtherYearTotals() {
	const { committee, year } = Route.useParams();
	const { data: balances } = useSuspenseQuery({ ...committeeBalancesOptions({ committee: committee.toUpperCase() }) });
...
function RouteComponent() {
	...
	const { data: user } = useSuspenseQuery(currDBUserQueryOptions);
	const member = user?.memberships.find((m) => m.committee.toLowerCase() === committee);
	const { data: stats } = useSuspenseQuery({ ...donationStatsOptions({
		committee: committee.toUpperCase(),
		year: year ?? config.activeYear
	}) });
	const { data: members } = useSuspenseQuery({ ...committeeMemberOptions({ committee: committee.toUpperCase() }) });
