...
import { ProfileSchemaWithValidation, cn, withMetaLogger } from "./user.schema-h0fSWBX3.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/separator-BonH_eRT.js
...
var getCurrUserFromDB = createServerFn({ method: "GET" }).middleware([authMiddleware, withMetaLogger("/users/me")]).handler(createSsrRpc("ea2096ddef8973c13ae78eddd783dedaaa1ea8414e916fdcb7d5082c9e2a6960"));
var createProfile = createServerFn({ method: "POST" }).middleware([assertAuthMiddleware, withMetaLogger("/users")]).validator(ProfileSchemaWithValidation).handler(createSsrRpc("a59e7d40c370ff50f23f6e28ca9aec6e2e73e9332a69829a3185e39a3894d49d"));
var updateProfile = createServerFn({ method: "POST" }).middleware([assertAuthMiddleware, withMetaLogger("/users")]).validator(ProfileSchemaWithValidation).handler(createSsrRpc("b4617b0c8cb9847ca1f963b245b23e76f90f829aa3b609d285ca01b7ade90cdd"));
var becomeMember = createServerFn({ method: "POST" }).middleware([assertAuthMiddleware, withMetaLogger("/members/commmittee/<name>")]).validator(CommitteeQuerySchema).handler(createSsrRpc("7e58e0865a71f0cfbdb369518d5cf2ccd125c8ccc3a32058fb5543aad08e0b4c"));
var getMembersByCommittee = createServerFn({ method: "GET" }).middleware([withMetaLogger("/members/committee/<name>")]).validator(CommitteeQuerySchema).handler(createSsrRpc("0531c07c91735e4f968b1439e6aed7ef155791e83bb1b6d173c6655ece5cd059"));
var getCurrUserBalancesByCommittee = createServerFn({ method: "GET" }).middleware([assertAuthMiddleware, withMetaLogger("/users/me/balances/committee/<name>")]).validator(CommitteeQuerySchema).handler(createSsrRpc("6e44fd450e5dd443132ed4115d107c7da6edd6786d9b723e24aba62e48a5869a"));
var getMemberBalancesByCommittee = createServerFn({ method: "GET" }).middleware([assertAuthMiddleware, withMetaLogger("/users/balances/committee/<name>")]).validator(CommitteeQuerySchema).handler(createSsrRpc("91f2d8dd2f7693eb7a4ecae53bd42656f6a859877cde4355972d007960781aed"));
