...
//#region node_modules/.nitro/vite/services/ssr/assets/txn.services-B6Gz9MCG.js
...
var getBalancesByCommittee = createServerFn({ method: "GET" }).validator(CommitteeQuerySchema).handler(getBalancesByCommittee_createServerFn_handler, async ({ data }) => {
	try {
...
var getDonationStatsByCommittee = createServerFn({ method: "GET" }).validator(CommitteeYearQuerySchema).handler(getDonationStatsByCommittee_createServerFn_handler, async ({ data }) => {
...
var getTransactionsByCommittee = createServerFn({ method: "GET" }).middleware([assertAuthMiddleware]).validator(TxnQuerySchema).handler(getTransactionsByCommittee_createServerFn_handler, async ({ data }) => {
	try {
...
var createTransaction = createServerFn({ method: "POST" }).validator(TransactionSchemaWithValidation).handler(createTransaction_createServerFn_handler, async ({ data }) => {
...
var updateTransaction = createServerFn({ method: "POST" }).middleware([assertAuthMiddleware]).validator(TransactionSchemaWithValidation).handler(updateTransaction_createServerFn_handler, async ({ data }) => {
...
var deleteTransaction = createServerFn({ method: "POST" }).middleware([assertAdminMiddleware]).validator(TransactionIDSchema).handler(deleteTransaction_createServerFn_handler, async ({ data }) => {
	try {
...
var getLinkedTransfer = createServerFn({ method: "GET" }).validator(v4_default.object({ txnId: v4_default.string().optional() })).handler(getLinkedTransfer_createServerFn_handler, async ({ data }) => {
	try {
...
var getItems = createServerFn({ method: "GET" }).validator(ItemQuerySchema).handler(getItems_createServerFn_handler, async ({ data }) => {
	try {
