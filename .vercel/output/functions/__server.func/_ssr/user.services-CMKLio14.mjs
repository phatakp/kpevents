...
import { ProfileSchemaWithValidation, withMetaLogger } from "./user.schema-BooD9qhh.mjs";
import { createServerRpc } from "./createServerRpc-DPX_ndmm.mjs";
import { api, handleAPIError } from "./axios-DW8fiHrc.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/user.services-CMKLio14.js
...
var becomeMember = createServerFn({ method: "POST" }).middleware([assertAuthMiddleware, withMetaLogger("/members/commmittee/<name>")]).validator(CommitteeQuerySchema).handler(becomeMember_createServerFn_handler, async ({ data }) => {
	try {
...
var getMembersByCommittee = createServerFn({ method: "GET" }).middleware([withMetaLogger("/members/committee/<name>")]).validator(CommitteeQuerySchema).handler(getMembersByCommittee_createServerFn_handler, async ({ data }) => {
	try {
...
var getCurrUserBalancesByCommittee = createServerFn({ method: "GET" }).middleware([assertAuthMiddleware, withMetaLogger("/users/me/balances/committee/<name>")]).validator(CommitteeQuerySchema).handler(getCurrUserBalancesByCommittee_createServerFn_handler, async ({ data }) => {
	try {
...
var getMemberBalancesByCommittee = createServerFn({ method: "GET" }).middleware([assertAuthMiddleware, withMetaLogger("/users/balances/committee/<name>")]).validator(CommitteeQuerySchema).handler(getMemberBalancesByCommittee_createServerFn_handler, async ({ data }) => {
	try {
