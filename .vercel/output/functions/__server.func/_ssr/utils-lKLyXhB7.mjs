import { BUILDING_FLOORS, COMMITTEE, DONATION_TYPE, TXN_MODE, TXN_TYPE } from "./common.schema-rOPsTdW8.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/utils-lKLyXhB7.js
var withMetaLogger = (url) => createMiddleware({ type: "function" }).client(async ({ next, method }) => {
	if ("production".includes("prod")) return await next({ sendContext: {
		method,
		url
	} });
	console.log(`[CLIENT] Calling ${method} ${url}`);
	return await next({ sendContext: {
		method,
		url
	} });
}).server(async ({ next, data, context }) => {
	if ("production".includes("prod")) return await next();
	try {
		const startTime = Date.now();
		console.log(`[SERVER] Calling ${context.method} ${context.url} ${data ? `with Arguments: ${JSON.stringify(data)}` : ""}`);
		const result = await next();
		const duration = Date.now() - startTime;
		console.log(`[SERVER] Finished ${context.method} ${context.url} in ${duration}ms`);
		return result;
	} catch (error) {
		console.warn(`[SERVER] Error ${context.method} ${context.url} ${JSON.stringify(error, null, 4)}`);
		throw error;
	}
});
function cn(...inputs) {
...
function isBookingType(type) {
	return type === DONATION_TYPE.ANNADAAN || type === DONATION_TYPE.TEMPLE_ITEM;
}
//#endregion
export { amountFormatter, amountShortener, cn, getDefaultFormOptions, getFilteredTxns, getFlatsForBuilding, getPaidByOptions, getStepAmount, getStepQty, getUserInfo, getUserOptions, isBookingType, isValidDate, withMetaLogger };
